	...
	static public function ctrUserLogin()
	{
		...
		if (isset($_POST["loginUser"])) {
			...
			if (
				...
				if (isset($answer["user"]) == isset($_POST["loginUser"]) && isset($answer["password"]) == isset($encryptpass)) {
		...
		if (isset($_POST["newUser"])) {
			...
			if (
				...
				echo '<script>
					...
					new swal({
						type: "error",
						title: "No especial characters or blank fields, try again",
						showConfirmButton: true,
						confirmButtonText: "Close"
			...
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["EditName"])) {
				...
				if ($_POST["EditPasswd"] != "") {
					...
					if (preg_match('/^[a-zA-Z0-9]+$/', $_POST["EditPasswd"])) {
						...
						echo '<script>
							...
							new swal({
								type: "error",
								title: "No especial characters in the password or blank fields, try again",
								showConfirmButton: true,
								confirmButtonText: "Close"
				...
				if ($answer == 'ok') {
					...
					echo '<script>
						...
						new swal({
							type: "error",
							title: "No especial characters in the name or blank field, try again",
							showConfirmButton: true,
							confirmButtonText: "Close"
							 }).then(function(result){
